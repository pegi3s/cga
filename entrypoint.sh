...
if [[ $# -gt 0 && "$1" == "--help" || "$1" == "-h" ]]; then
	...
	echo -e "\t- process_logs.sh /working_dir /working_dir/logs/<timestamp>/tasks: processes the Compi logs directory to create a report of tasks with errors or warnings."
else
	if [ $# -gt 0 ] && [ "$1" == "resume" ]; then
		/compi resume -p /pipeline*.xml "${@:2}"
	else
		if [ $# -gt 0 ] && [ "$1" == "init_working_dir.sh" ]; then
			init_working_dir.sh "${@:2}"
		else
